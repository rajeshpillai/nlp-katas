...
for example in syntactic_examples:
    ...
    for i, reading in enumerate(example["readings"], 1):
...
def naive_search(documents, query):
    """Search documents by simple word matching."""
    results = []
...
results = naive_search(documents, "bank")
print("--- Naive search for 'bank' ---\n")
print(f"Query: 'bank'")
print(f"Results: {len(results)} documents matched")
for r in results:
    print(f"  - {r}")
print()
print("Problem: ALL documents matched.")
print("The search cannot tell financial 'bank' from river 'bank'.")
print("This is why NLP needs more than word matching.")
```
